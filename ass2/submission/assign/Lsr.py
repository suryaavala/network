...
link = []
for k in sorted(neighbour.keys()):
    link.append((min(node_ID,k),max(node_ID,k),neighbour[k][0]))
my_lsr[node_ID] = link
...
def ls_advertisement():
    ...
    while True:
        ...
        for i in dead_neighbours:
            #reset network
            print ('Node {} is no longer alive!'.format(i))

            ###->
            for d in dead_neighbours:
                if d in neighbour.keys():
                    del neighbour[d]
            dead_neighbours[:] = []
            my_lsr = {}
            link = []
...
def listen():
    ...
    for i in neighbour.keys():
        neighbour_timer[i] = time.time()

    while True:
        # if len(net_graph.getEdges()) == 10:
        #     print ('{} is done!'.format(node_ID))
        for t in neighbour_timer:
            if time.time()-neighbour_timer[t]>=death_time:
                if t not in dead_neighbours:
                    dead_neighbours.append(t)
        for d in dead_neighbours:
            if d in neighbour_timer.keys():
                del neighbour_timer[d]
        ...
        try:
            ...
            for p in pack:
                if p in lsr and lsr[p] == pack[p]:
                    if p in neighbour.keys():
                        neighbour_timer[p] = time.time()
                    continue
                # elif p in lsr:
                #     dead = []
                #     for k in lsr[p]:
                ...
                #     for dd in dead:
                #         del lsr[dd]
                #         net_graph.nodeDead(dd)
                #         print ('Node {} is no longer alive!'.format(dd))
                else:
                    lsr[p] = pack[p]
