...
#error handling can be added
    ...
    def _transmit(self, packets):
        ...
        print (self.received_acknb,last_packet,self.seq_nb,sorted(packets))
...
#event1:            #send packets until mws is reached
            while (int(self.seq_nb)-int(self.received_acknb)-1) <= self.mws and (int(self.seq_nb)<=last_packet):
